...
In Figure 2, we observe that exponential behavior of surviving probability following a sharp initial drop agrees with our interpretation of the prevalence graph. The virus prevalence plots in Figure 1 and the surviving probability plots in Figure2 are compatible because, for a fixed spreading rate (λ = 0.065), the surviving probability must proportionally decrease as the network size increases to yield the same level of virus prevalence for all network sizes. While we confirm a qualitative match between the original figure and our result of the simulation, we do observe a quantitative difference. The surviving probability in our plot for N < 5 × 10<sup>5</sup> decreases more rapidly, roughly twice as faster, than that of the original. We do observe in our simulation that running more trials results in a graph that is more quantitatively similar to the original. So this quantitative difference potentially comes from an insufficient number of trials. However, we are unable to identify the exact cause of this discrepancy because Pastor-Satorras and Vespignani [1] did not explicitly mention the number of trials they used.


### Going Beyond: SIS model simulation on Facebook network

To extend Pastor-Satorras and Vespignani's experiment beyond the BA graph, we apply the same implementation of the SIS model simulation to a Facebook network with 88234 edges and 4039 nodes. Assuming that the Facebook graph is similar to the BA graph, we expect the same `ρ ~ exp(-C/λ)` prevalance plot as shown in Figure 1.

We observe that running the same simulation on the Facebook data yields the results shown in Figure 3. We confirm that the result qualitatively matches the prevalence plot shown in Figure 1 but quantitatively different in regards to the slope of the line and the prevalence level.

We know from Downey that a BA graph that resembles the Facebook dataset with 88374 edges and 4039 nodes differs greatly in the clustering coefficient. The clustering coefficient is 0.037 for the BA graph and 0.061 for the Facebook graph [5]. If the only major difference between the BA graph and the Facebook graph is the clustering coefficient, we can assume that the difference in the clustering coefficient is the cause of quantitative discrepancy shown in Figure 3. Even if the assumption is not completely true, we can still assume that the clustering coefficient is one of the major factors that contribute to the higher prevalence level and the relatively low prevalence constant C value compared to the BA graph.

With higher prevalence level, viruses can stay more prevalent in the Facebook network with more infected nodes than in the BA graph. With a smaller value of the prevalence constant C, the change in spreading rate has less effect on exponentially decaying prevalence. So viruses can pervade in the Facebook network more dominantly than in the BA graph.
...
We conclude that scale-free networks are generally prone to epidemic spreading regardless of the spreading rate because there always exists a finite prevalence that is greater than zero. However, the epidemic prevalence decays exponentially as the spreading rate increases, so the epidemic prevalence is generally low for a wide range of spreading rate. In other words, only a small number of nodes are going to be infected at a moment, but the epidemic will continue. We also identified a potential positive correlation between the epidemic prevalence and the clustering coefficient of a network by investigating the SIS model on the Facebook network. It is, in fact, trivial that a more clustered network yields more infections as in observed on many occasions in nature. For future works, we suggest you to explore the topic of network clustering to better clarify the relationship between the epidemic prevalence and the clustering coefficient.
